...
read_csv("./data/macro-economic_indicators.csv", skip = 2) %>% 
...
growth_gdp_sectoral <- read_csv("./data/economic_growth_rates_sectors.csv", skip = 1) %>% 
  select(-1) %>% 
...
read_csv("./data/area_production_productivity/cereal_legumes_production_area_nepal.csv", skip = 1) %>% 
...
read_csv("./data/area_production_productivity/commerical_production_area_nepal.csv", skip = 1) %>% 
  filter(source %in% c("Agriculture diary, 2076", "Agriculture diary, 2076")) %>% 
...
read_csv("./data/area_production_productivity/vegetable_fruits_production_area_nepal.csv", skip = 1) %>% 
  filter(source %in% c("Agriculture diary, 2076", "Agriculture diary, 2076")) %>% 
...
read_csv("./data/poverty_related_key_indicators_2020.csv", skip=2) %>% 
...
read_csv("./data/key_employment_indicators_2020.csv", skip=2) %>% 
...
read_csv("./data/minimum_support_price_commodities_2020.csv", skip = 3) %>% 
  select(-2) %>% 
...
read_csv("./data/status_of_agricultural_crops_production_2020.csv", skip = 3) %>% 
...
read_csv("./data/status_of_credit_services_to_livestock_and_vegetables_2020.csv", skip = 2) %>% 
...
read_csv("./data/indicators_of_educational_achievements_2020.csv", skip = 2) %>% 
  knitr::kable(caption = "Educational achievements of recent years; Source: Economic Survey, 2019/20", longtable = TRUE, booktabs = TRUE, escape = FALSE) %>% 
  kableExtra::kable_styling(latex_options = "striped", font_size = 8) %>% 
  # kableExtra::collapse_rows(columns = 1) %>%
  kableExtra::column_spec(column = 1:8, width = c("10em", "8em", rep("4em", 6)))
```
...
tribble(
  ...
  knitr::kable(booktabs = TRUE, caption = "Climate change scenario in Nepal and some indicators") %>% 
  kableExtra::kable_styling(latex_options = "striped", font_size = 8) %>% 
  kableExtra::column_spec(1:4, width = c("3em", "6em", "5em", "4em"))
```

- For every 100 m rise in altitude, the mean annual temperature drop sby -\SI{0.5}{\celsius}.
...
tribble(
  ...
  knitr::kable(booktabs = TRUE, caption = "Layers of atmosphere") %>% 
  kableExtra::kable_styling(latex_options = "striped", font_size = 8) %>% 
...
tribble(
  ...
  knitr::kable(caption = "Worldwide emmission estimates of different sectors of economy. Source: IPCC, 4th Assessment Report 2007")
```
...
tribble(
 ...
 \\end{itemize}"
  ...
  kableExtra::column_spec(column = 1:2, width = c("10em", "24em"))
```
...
tribble(
  ...
  knitr::kable(booktabs = TRUE, caption = "Membership constiution of NARC Council") %>% 
  kableExtra::kable_styling(font_size = 8, latex_options = "striped") %>% 
...
tribble(
  ...
  knitr::kable(booktabs = TRUE, caption = "Constitution of Executive Committee of NARC") %>% 
  kableExtra::kable_styling(font_size = 8, latex_options = "striped") %>% 
  kableExtra::column_spec(column = 1, width = "18em")
```
